...
methyl_form_differential_sort_pavlue <- function(differential_methyl_from, pileup_position, specified_region){
  differential_methyl_from_pvalue <- differential_methyl_from %>%
    group_by(chrom, id, group) %>%
    slice_min(., balanced_map_pvalue) %>%
    group_by(chrom, group) %>%
    distinct(id, .keep_all = T) %>%
    select(chrom, start, end, specific_id_strand, balanced_map_pvalue, cohen_h, differential, id, locus_tag, symbol, group)
...
write_tsv(gene_body_promoter_sort_pavluel_combine, 
          file=glue("{out_dir}/{a_group}_{b_group}_{phase_short}_{dataset}_gene_body_promoter_sort_pvalue.tsv"))
